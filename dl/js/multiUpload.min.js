var origin = document.location.origin,
    ...
    addFiles = function(e) {
        ...
        for (dropzone.css("overflow", "auto"), checkItems(), x = 0; x < e.length; x += 1) {
            ...
            _ += '<p class="fileName"><span class="glyphicon glyphicon-cloud-upload"></span> ' + e[x].name.substr(0, 100) + d + '</p>';
            _ += '<span id="cancel-button-' + c + '" style="float: right" onclick="myRemove(' + c + ')"><button class="btn btn-xs btn-danger">Remove</button></span>';
            _ += '</div>', _ += '<div class="clearfix"></div>';
            _ += '<div class="col-md-12" style="margin: 0;">', _ += '<span id="storage-' + c + '" class="storage">' + _convertSize(e[x].size) + "</span>", _ += '<div class="speed" id="speed-' + c + '"></div>', _ += '<div class="progress">', _ += '<div id="progress-bar-' + c + '" class="progress-bar progress-bar-striped active"></div>', _ += "</div>", _ += "</div>", t.innerHTML = _, dropzone.append(t);
            _ = "";
        }
    },
    addItems = function(e) {
    ...
    myRemove = function(e) {
        ...
        for (t = e; t < allItems.length; t++) {
          ...
          $("#cancel-button-" + (t + 1)).attr("onClick", "myRemove(" + t + ");");
    ...
    myPause = function(e) {
      ...
      $('<span id="resume-button-'+e+'" style="float: right" onclick="myResume('+e+')"><button class="btn btn-xs btn-success">Resume</button></span>').insertBefore("#cancel-button-"+e);
      $("#progress-bar-" + e).removeClass("progress-bar-striped active progress-bar-warning").addClass("progress-bar-info");
    };
    myResume = function(e) {
      $("#resume-button-" + e).remove();
      $('<span id="pause-button-'+e+'" style="float: right" onclick="myPause('+e+')"><button class="btn btn-xs btn-primary">Pause</button></span>').insertBefore("#cancel-button-"+e);
...
multiUpload.prototype.upload = function() {
    ...
    if (200 == e.target.status || 201 == e.target.status) {
        var _ = JSON.parse(e.target.response),
            c = "type=savefile&fileId=" + _.id + "&fileName=" + _.name + "&fileType=" + _.mimeType + "&fileSize=" + this.file.size;
        new _request({
            method: "POST",
            url: "./upload",
            data: c,
...
var mediaUpload = function(e, x) {
    ...
    uploadFile = function(e, x) {
        if (void 0 != typeof e[x]) {
            ...
            if (e[x].size < 1) return $("#progress-bar-" + x + ", #cancel-button-" + x).remove(), $("#storage-" + x).html('<strong style="color:red">Error: Tệp tin trống</strong>'), x + 1 < e.length ? uploadFile(e, x + 1) : allItems.splice(0, allItems.length);
            //$('<span id="pause-button-'+x+'" style="float: right" onclick="myPause('+x+')"><button class="btn btn-xs btn-primary">Pause</button></span>').insertBefore("#cancel-button-"+x);
            //return;
            _checkUpload(e, x)
        }
    },
    _checkUpload = function(e, x) {
		    var params = 'type=checkfileupload&fileName='+encodeURIComponent(e[x].name)+'&fileType='+encodeURIComponent(e[x].type)+'&fileSize='+e[x].size;
...
sendError = function(type,data) {
  $("#progress-bar-" + myStart).removeClass("progress-bar-striped active progress-bar-warning").addClass("progress-bar-danger").html("Có lỗi xảy ra.");

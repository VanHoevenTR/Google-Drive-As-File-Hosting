...
multiUpload.prototype.upload = function() {
    ...
    if (200 == e.target.status || 201 == e.target.status) {
        ...
        new _request({
            ...
            onError: function(e) {
                $("#progress-bar-" + myStart).removeClass("progress-bar-striped active progress-bar-warning").addClass("progress-bar-danger").html("Có lỗi xảy ra, vui lòng thử lại sau."), xhr_arr.length = 0, x.onComplete()
            }
        })
...
var mediaUpload = function(e, x) {
    ...
    uploadFile = function(e, x) {
        if (void 0 != typeof e[x]) {
            if (e[x].size > 107374182400) return $("#progress-bar-" + x + ", #cancel-button-" + x).remove(), $("#storage-" + x).html('<strong style="color:red">Error: File lớn hơn 100gb</strong>'), x + 1 < e.length ? uploadFile(e, x + 1) : allItems.splice(0, allItems.length);
            _checkUpload(e, x)
        }
    },

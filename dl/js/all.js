var click, storage, balance, domain = (typeof root !== 'undefined') ? root : window.location.origin;
$("img.thumb").click(function() {
    var e = $(this)[0].src;
    $(".preview").html('<img src="' + e + '" width="100%">')
});
var folder_create = function() {
    ...
    folder_info = function(e) {
        if (!e || click) return !1;
        click = !0;
        var a = "type=folder_info&id=" + e;
        $("#folder_update").show(), $(".manager_info_subscene, .manager_info_embed, #manager_update").hide(), new _request({
            method: "POST",
            dataType: "json",
            data: a,
            onComplete: function(e) {
                if (e.status && e.folder[0]) $("#manager_info_embed").val(""), $("#manager_info_title").text(e.folder[0].name), $("#manager_info_id").val(e.folder[0].id), $("#manager_info_name").val(e.folder[0].name), $("#manager_info_password").val(e.folder[0].pass), $("#manager_info_share").val(domain + "/folder/" + e.folder[0].slug);
                else {
    ...
    folder_update = function() {
        if (id = $("#manager_info_id").val(), name = $("#manager_info_name").val(), password = $("#manager_info_password").val(), !id || !name || name.length < 3 || click) return flase;
        click = !0;
        var e = "type=folder_update&id=" + id + "&name=" + name + "&password=" + password;
        new _request({
            method: "POST",
            dataType: "json",
            data: e,
...
var manager_info = function(e) {
    ...
    manager_update = function() {
        if (id = $("#manager_info_id").val(), name = $("#manager_info_name").val(), password = $("#manager_info_password").val(), subscene = $("#manager_info_subscene").val(), !id || !name || name.length < 3 || click) return flase;
        click = !0;
        var e = "type=manager_update&id=" + id + "&name=" + name + "&password=" + password + "&subscene=" + subscene;
        new _request({
            method: "POST",
            dataType: "json",
            data: e,
    ...
    manager_delete = function(e) {
        ...
        if (confirm("Bạn có chắc muốn xóa?")) {
            click = !0;
            var a = "type=manager_delete&id=" + e;
            new _request({
                method: "POST",
                dataType: "json",
                data: a,
    ...
    manager_delete_choose = function() {
        if (confirm("Bạn có muốn xóa danh sách File đã chọn?")) {
            ...
            click = !0;
            for (var a = 0, t = e.length; t > a; a++) e[a].checked && (fileIds += e[a].value + ",", listItems.push(e[a].value));
            var n = "type=manager_delete_all&fileIds=" + fileIds;
            new _request({
                method: "POST",
                dataType: "json",
                data: n,
...
$(function() {
    $("#remote_links").keyup(function() {
        ...
        for ($(".msg-danger").hide(), i = 0; i < e.length; i++)
            if (a[i]) {
				check = a[i].match(/^(?:https?:\/\/)?(?:www\.)?(?:(?:drive\.google|googledrive)\.com\/)(?:file\/d\/|a\/[^\/]+\/file\/d\/|host\/|open\?id=)([\w-]{10,40})(?:\?|\/)?/), check || remote_changer(a, i, check), 
				parseInt(a.length - 1) == i && (keyup = click = !1, $("#remote_links").prop("disabled", !1))
...
var listLink = [],
    ...
    remote_start = function(e, a) {
        e[a] || remote_next(e, a), fileId = e[a].match(/^(?:https?:\/\/)?(?:www\.)?(?:(?:drive\.google|googledrive)\.com\/)(?:file\/d\/|a\/[^\/]+\/file\/d\/|host\/|open\?id=)([\w-]{10,40})(?:\?|\/)?/), fileId || remote_next(e, a), $(".remote-loading").show(), new _request({
            method: "GET",
            url: "https://www.googleapis.com/drive/v3/files/" + fileId[1] + "?fields=mimeType%2Cname%2Csize%2Cparents%2Cmd5Checksum&key=" + apiKey,
            dataType: "json",
            onComplete: function(t) {
                if (fileType = t.mimeType.split("/"), "application/vnd.google-apps.folder" == t.mimeType) {
                    var n = "type=remote_folder&fileId=" + fileId[1];
                    return new _request({
                        method: "POST",
                        dataType: "json",
                        data: n,
                        onComplete: function(t) {
                            ...
                            else $(".remote_response").append('<tr><td class="col-md-3 middle text-center"><a href="' + e[a] + '" target="_blank">' + fileId[1] + '</a></td><td class="col-md-6 middle text-center">n/a</td><td class="col-md-3 middle text-center" style="color:red">' + t.msg + "</td></tr>"), remote_next(e, a)
                        },
                        onError: function() {
                            $("#response, .response").show(), $(".remote_response").append('<tr><td class="col-md-3 middle text-center"><a href="' + e[a] + '" target="_blank">' + fileId[1] + '</a></td><td class="col-md-6 middle text-center">n/a</td><td class="col-md-3 middle text-center" style="color:red">Lỗi server</td></tr>'), remote_next(e, a)
                        }
                    }), !0
                }
                if (!t.md5Checksum) return $("#response, .response").show(), $(".remote_response").append('<tr><td class="col-md-3 middle text-center"><a href="' + e[a] + '" target="_blank">' + fileId[1] + '</a></td><td class="col-md-6 middle text-center">n/a</td><td class="col-md-3 middle text-center" style="color:red">Tệp tin không an toàn</td></tr>'), remote_next(e, a), !1;
                var n = "type=remote&fileId=" + fileId[1] + "&mimeType=" + t.mimeType + "&fileName=" + t.name + "&fileSize=" + t.size + "&md5Checksum=" + t.md5Checksum;
                new _request({
                    method: "POST",
                    dataType: "json",
                    data: n,
                    onComplete: function(t) {
						var file_info = t.info || "";
                        //$("#response, .response").show(), t.status ? ($(".response textarea").val($(".response textarea").val() + t.msg + "\n"), $(".remote_response").append('<tr><td class="col-md-3 middle text-center"><a href="' + e[a] + '" target="_blank">' + fileId[1] + '</a></td><td class="col-md-6 middle text-center"><a href="' + t.msg + '" target="_blank">' + t.msg + '</a></td><td class="col-md-3 middle text-center" style="color:green">Thành công</td></tr>')) : $(".remote_response").append('<tr><td class="col-md-3 middle text-center"><a href="' + e[a] + '" target="_blank">' + fileId[1] + '</a></td><td class="col-md-6 middle text-center">n/a</td><td class="col-md-3 middle text-center" style="color:red">' + t.msg + "</td></tr>"), remote_next(e, a)
                        $("#response, .response").show(), t.status ? ($(".response textarea").val($(".response textarea").val() + t.msg + "\n"), $(".remote_response").append('<tr><td class="col-md-3 middle text-center"><a href="' + e[a] + '" target="_blank">' + fileId[1] + '</a></td><td class="col-md-6 middle text-center"><a href="' + t.msg + '" target="_blank">' + t.msg + '</a></td><td class="col-md-3 middle text-center" style="color:green">'+(file_info!='' ? file_info : 'Thành công')+'</td></tr>')) : $(".remote_response").append('<tr><td class="col-md-3 middle text-center"><a href="' + e[a] + '" target="_blank">' + fileId[1] + '</a></td><td class="col-md-6 middle text-center">n/a</td><td class="col-md-3 middle text-center" style="color:red">' + t.msg + "</td></tr>"), remote_next(e, a)
                    },
                    onError: function() {
                        $("#response, .response").show(), $(".remote_response").append('<tr><td class="col-md-3 middle text-center"><a href="' + e[a] + '" target="_blank">' + fileId[1] + '</a></td><td class="col-md-6 middle text-center">n/a</td><td class="col-md-3 middle text-center" style="color:red">Có lỗi xảy ra, vui lòng thử lại sau</td></tr>'), remote_next(e, a)
                    }
                })
            },
            onError: function(t) {
                var n = JSON.parse(t.responseText);
                $("#response, .response").show(), $(".remote_response").append('<tr><td class="col-md-3 middle text-center"><a href="' + e[a] + '" target="_blank">' + fileId[1] + '</a></td><td class="col-md-6 middle text-center">n/a</td><td class="col-md-3 middle text-center" style="color:red">' + n.error.message + "</td></tr>"), remote_next(e, a)
            }
        })
    },
    ...
    group_status = function(e, a) {
        if (!e || click) return !1;
        click = !0;
        var t = "type=group_status&id=" + e + "&status=" + a;
        new _request({
            method: "POST",
            dataType: "json",
            data: t,
    ...
    mmo = function() {
        ...
        if (csrf = $('meta[name="csrf-token"]').attr("content"), !csrf || click) return !1;
    ...
    withdraw = function() {
        var e = $("#note").val();
        if (choose = $("#choose-gifts").val(), mobile_network = $("#mobile-network").val(), mobile_rates = $("#mobile-rates").val(), mobile_receiver = $("#mobile-receiver").val(), game_network = $("#game-network").val(), game_rates = $("#game-rates").val(), game_receiver = $("#game-receiver").val(), paypal_email = $("#paypal-email").val(), paypal_rates = $("#paypal-rates").val(), banking_network = $("#banking-network").val(), banking_network_option = $("#banking-network-option").val(), banking_account_number = $("#banking-account-number").val(), banking_name = $("#banking-name").val(), banking_rates = $("#banking-rates").val(), csrf = $('meta[name="csrf-token"]').attr("content"), !(1 != choose || csrf && mobile_network && mobile_rates)) return !1;
        if (!(2 != choose || csrf && game_network && game_rates)) return !1;
        if (!(3 != choose || csrf && paypal_email && paypal_rates)) return !1;
        if (!((4 != choose || csrf && banking_network && banking_account_number && banking_name && banking_rates) && (10 != banking_network || banking_network_option))) return !1;
...
_request.prototype._send = function() {
    ...
    $(".loading").show(), $(".loading").html('<img src="'+domain+'/imgs/radio.gif">'), $.ajax({
        type: e.method,
        url: e.url,
        headers: e.headers,
        dataType: e.dataType,
